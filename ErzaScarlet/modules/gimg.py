...
from ErzaScarlet.utils import pool, Message

from ErzaScarlet.utils.upload.upload import doc_upload, photo_upload
...
async def gimg_down(message: Message):
    ...
    if upload_:
        ...
        time_taken_s = (end_t - start_t).seconds
        await message.edit(
            f"Downloaded {limit} {media_type} to `{os.path.join("downloads/", text)}` in {time_taken_s}"
...
def check_path(path_name: str = "GIMG"):
    path_ = os.path.join("downloads/", path_name)
