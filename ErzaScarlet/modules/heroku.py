...
@bot.on(pattern="restart$")
@bot.on(pattern="restart$", allow_sudo=True)
async def re(hell):
    if hell.fwd_from:
        return
...
@bot.on(pattern="svar ?(.*)")
@bot.on(pattern="svar ?(.*)", allow_sudo=True)
...
@bot.on(pattern="usage(?: |$)", outgoing=True)
@bot.on(pattern="usage(?: |$)", allow_sudo=True)
